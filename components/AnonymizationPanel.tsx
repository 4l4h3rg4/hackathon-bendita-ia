  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="p-4 md:p-6">
        <div className="overflow-x-auto rounded-lg border border-gray-200 mb-8 hidden md:block">
        ...
        <div className="md:hidden space-y-4 mb-8">
          {data.map((row) => (
            <div key={row.id} className="border border-gray-200 rounded-xl overflow-hidden">
              ...
              <div className="bg-red-50/50 p-3 border-b border-gray-100 flex items-center gap-2 text-red-800 font-medium text-sm">
                <Eye className="w-4 h-4" />
                <span>Datos Originales</span>
              </div>
              <div className="p-4 bg-white space-y-1 text-sm text-gray-600 font-mono border-b border-gray-100">
                <p><span className="font-bold text-gray-700">Nombre:</span> {row.original.nombre}</p>
                <p><span className="font-bold text-gray-700">RUT:</span> {row.original.rut}</p>
                <p><span className="font-bold text-gray-700">Dx:</span> {row.original.diagnostico}</p>
                <p className="text-gray-400"><span className="font-bold text-gray-700">Dir:</span> {row.original.direccion}</p>
              </div>

              {/* Anonymized Data Header */}
              <div className="bg-green-50/50 p-3 border-b border-gray-100 flex items-center gap-2 text-green-800 font-medium text-sm">
                <Lock className="w-4 h-4" />
                <span>Vista Anonimizada</span>
              </div>
              <div className="p-4 bg-green-50/10 space-y-1 text-sm text-gray-600 font-mono">
                <p><span className="font-bold text-green-700">Nombre:</span> {row.anonymized.nombre}</p>
                <p><span className="font-bold text-green-700">RUT:</span> {row.anonymized.rut}</p>
                <p><span className="font-bold text-green-700">CIE-10:</span> {row.anonymized.diagnostico}</p>
                <p className="text-gray-400"><span className="font-bold text-green-700">Geo:</span> {row.anonymized.direccion}</p>
